...
export default class SE extends React.Component{
    constructor(props){
        ...
        this.state = {
            ...
            sLoading: true,
            exp_id: this.props.data.e,
            sel_a1: [],
            sel_a2: []
        }
    }
    componentDidMount(){
        document.title = this.state.exp_id;
        const url = `${REACT_APP_API_URL}/api/audit/get/exe/info`
        const p = {
                "cpykey": this.state.cpykey,
                "audit_1_id": this.state.audit_1_id,
                "audit_2_id": this.state.audit_2_id,
                "el": this.state.exp_id
            }
    ...
    render(){
        ...
        return(
            ...
            <h4>Exception: {this.state.exp_id}</h4>
            {single_dev}
          </div>
        )
    }
