...
export default class Search extends React.Component{
    constructor(props){
        ...
        this.state = {
            ...
            dev_list: this.props.data.dev_list,
            sLoading: true,
            show:"hide",
            sel_div:"",
            sel_a1: [],
    ...
    render(){
        ...
        <div className="scroll-heads-search">
            <span className="icon-search icon-size-24" style={{marginTop:"10px"}}></span><input type="text" value={this.state.searchFilter} onChange={this.handleSearch} placeholder="Search Device ...."/>
        </div>;
        const filter = this.state.dev_list.filter((e)=>{
            e = e.toLowerCase();
            return e.indexOf(this.state.searchFilter.toLowerCase()) !== -1
        })
        const show_search_dev = 
        <div>
            <div className="devices-scroll-head">
                <p>Devices: {filter.length}</p>
            </div>
            <div className="dev-grid">
                {filter.map((e)=>{
                    e = e.replace("\%","\%25")
                    return(
                        <div className="dev-grid-item">
                                <a target="_blank" rel="noreferrer" 
                                href={`/audit/device/${this.state.cpykey}/${this.state.audit_1_id}/${this.state.audit_2_id}/${e}`} 
